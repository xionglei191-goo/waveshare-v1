...
- `english`: `courses/english/` with JSON containing `title` and at least one
  of `lessons`, `phrases`, or `prompt`.
...
`..`. Files are written atomically and indexed with SHA-256, size, content type,
and version. Sample, test, diagnostic, demo, mock, and placeholder namespaces
are excluded from production views and cannot satisfy external acceptance.
