...
void BoxAudioCodec::OutputData(std::vector<int16_t>& data) {
    const int volume = std::clamp(output_volume_, 0, 100);
    if (volume <= 0) {
        std::fill(data.begin(), data.end(), 0);
    } else if (volume < 100) {
        const int32_t volume_factor = static_cast<int32_t>(volume * volume * 65536 / 10000);
        for (auto& sample : data) {
            sample = static_cast<int16_t>((static_cast<int32_t>(sample) * volume_factor) >> 16);
        }
    }
