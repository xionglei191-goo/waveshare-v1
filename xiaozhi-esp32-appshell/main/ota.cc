...
constexpr const char* kDefaultPrimaryOtaUrl = "http://192.168.31.246:8003/xiaozhi/ota/";
#endif
 ...
 */
esp_err_t Ota::CheckVersion(const std::string& url_override) {
    auto& board = Board::GetInstance();
    ...
    std::string url = url_override.empty() ? GetCheckVersionUrl() : url_override;
    if (url.length() < 10) {
