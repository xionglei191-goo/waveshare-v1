...
5. **后续消息交互**  
   ...
   - 在代码里，接收回调主要分为：  
     - `OnData(...)`:  
       ...
       - 当 `binary` 为 `false` 时，认为是 JSON 文本，需要在设备端用 cJSON 进行解析并做相应协议处理（如聊天状态、TTS、MCP 协议消息等）。  
...
WebSocket 文本帧以 JSON 方式传输，以下为常见的 `"type"` 字段及其对应协议语义。若消息里包含未列出的字段，可能为可选或特定实现细节。
...
- **JSON 消息**：使用 `"type"` 为核心字段标识不同协议语义，包括 TTS、STT、MCP、WakeWord、System、Custom 等。  
