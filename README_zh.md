...
本仓库是 Family AI OS 唯一有效的项目根目录，包含微雪 ESP32-S3 Touch LCD
1.85B 固件、Family Hub 后端和自建语音服务。原来的
`xiaozhi-esp32-appshell/` 嵌套目录及实验性 PlatformIO 工程均已弃用。

顶层目录约定如下：

- `main/`：ESP-IDF 固件和 AppShell 界面。
- `server/`：Family Hub 后端、管理页面、语音 Provider 接入、部署脚本和测试。
- `docs/family-ai-os/`：产品、架构、硬件、API 与验收文档。
- `scripts/`：固件构建及项目维护工具。
- `todo.md`：当前计划和历史实施记录。

固件命令统一在仓库根目录执行，后端命令统一在 `server/` 执行。开发前请先阅读
[AGENTS.md](AGENTS.md) 和
[当前状态](docs/family-ai-os/current-state.md)。

固件继续使用已经验证的小智语音技术栈作为基础，但本仓库此后独立开发，不再维护用于合并上游代码的第二套工作目录。
