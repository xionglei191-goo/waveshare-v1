...
class BuildPatchTests(unittest.TestCase):
    def test_patcher_parses_and_redacts_connection_headers(self):
        ...
        self.assertIn("ALI_ASR_EXPECTED_SHA256", source)
        self.assertIn("LISTEN_EXPECTED_SHA256", source)
        self.assertIn("trace_llm_stream", source)
        self.assertIn("self._connection_lock = asyncio.Lock()", source)
        ...
        self.assertIn("family_use_tools", source)
        self.assertIn("FAMILY_TTS_PREWARM", source)
        self.assertIn("LISTEN_IMPORT_REPLACEMENT", source)
