...
The AppShell connectivity layer reports BluFi as the `ble` provider when `CONFIG_USE_ESP_BLUFI_WIFI_PROVISIONING=y`, with the advertised device name `Xiaozhi-Blufi`. The Settings page shows this provider next to Wi-Fi/USB/cellular status so provisioning state is visible without changing business page code.
...
After a successful AppShell provisioning flow, the firmware posts a dedicated
`appshell.wifi_provisioned` device log to the backend. The payload includes
method `blufi`, SSID/IP/RSSI/channel from the board JSON, wake/reset context,
heap data and backend probe status. It never includes the Wi-Fi password.
...
- The Family Hub readiness endpoint requires a recent heartbeat plus the
  dedicated `appshell.wifi_provisioned` event before BluFi readiness becomes
  ready. This is only a readiness signal; the acceptance checkbox still needs
  real phone-side and serial-log evidence.
