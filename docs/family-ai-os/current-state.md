...
- `/Users/xionglei/Documents/waveshare` is the only local project root.
- Firmware lives in `main/`, Family Hub and Voice Provider live in `server/`, and product documentation lives in `docs/family-ai-os/`.
- The former `xiaozhi-esp32-appshell/` nesting and root PlatformIO starter have been retired. Do not recreate either project layout.
- The firmware keeps its verified Xiaozhi-derived voice stack, but development is now independent and no longer reserves a second tree for upstream merges.
...
- Low-latency voice routing (2026-07-12): the 246 Provider runs `xiaozhi-voice-provider:0.9.5-family-router5`. It keeps Page Agent First, disables hidden reasoning only for `deepseek-v4-flash`, limits lightweight replies to 160 tokens, skips the full functions schema after an explicit Family Hub `handled=false`, unwraps provider DSML before TTS, prewarms the Aliyun TTS WebSocket during recording, and retains tools only when Family Hub is unavailable. The pinned build verifies the upstream route, LLM, ASR, TTS and listen-handler SHA values.
- ESP voice latency telemetry (2026-07-12): `tts start` now prepares decoder/I2S output synchronously and audio acceptance uses an atomic playback-ready flag instead of waiting for the asynchronous `Speaking` UI transition. Opus codec priority is 5. Heartbeats include TTS-to-packet, packet-to-PCM, queue peaks, dropped packets, WebSocket send failures, total heap and internal SRAM. The recurring 12 KB backend refresh worker is persistent and the Family Hub HTTP client reuses keep-alive connections.
- Final AI latency release (2026-07-12): Family Hub runs `/opt/xiaozhi-family-hub/releases/20260712121100-ai-latency-final`; router5 image SHA is `fa4f3bcdea5c86a313a31f3b1d904098f29ca1ead6a025cc52f330e18839cb27`. Firmware binary SHA is `2b57bd21bedcbfb958d48b9e29536cba81a6a66a9e096abb81012e0f0848a877` and ELF SHA is `c22b0bf3e6598998a3cfb52a0ec53366542f804d912ed5e057c85f0df55dc354`. The fixed 10/8/2 real-device sample measured deterministic P50 `1.498s`, lightweight P50/P95 `3.317s/4.370s`, complex P95 `8.814s`, and packet-to-PCM P95 `36ms`, with no first-packet or decode drops. A 121-minute warmed stability window changed total heap by `+2064` bytes, held minimum internal SRAM at `68719` bytes, and stayed on boot #108 without panic or restart. Realtime mode has no explicit device `listen stop`, so those durations remain `-1` instead of being inferred.
