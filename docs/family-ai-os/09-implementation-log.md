...
- 完成 AI 全链路与 ESP32 延迟优化：天气语音改用 35 分钟缓存，显式刷新最多等待 800 ms；后台增加 AI 延迟分层 P50/P95 和真实内部 SRAM/设备首个 PCM 指标。
- Voice Provider 升级到固定 SHA 构建的 `router5`：轻量模型关闭隐藏推理并限制 160 tokens，普通回退跳过完整工具 schema，修复 Aliyun ASR 手动结束 race，录音期预热 TTS，并过滤 DSML 协议标记。
- ESP32 修复 `tts start` 与首包竞争，Opus 任务优先级提升到 5；新增每轮语音时延、队列、丢包、发送失败和首包内存指标；后台 refresh 改为常驻任务，HTTP client 改为 keep-alive 复用。
- 最终真机 10/8/2 轮验收中，确定性、lightweight、complex 首音频 P50 分别为 `1.498s`、`3.317s`、`6.680s`，对应 P95 为 `4.412s`、`4.370s`、`8.814s`；设备首包到 PCM P95 为 `36ms`，无首包/解码丢包。warm-up 后连续 121 分钟总 heap 净变化 `+2064` 字节，最低内部 SRAM `68719` 字节，同一 bootId 且无 panic 或重启。
