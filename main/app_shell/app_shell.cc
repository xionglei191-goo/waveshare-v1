...
// 空闲省电梯度阈值(秒)。60s 降暗、120s 熄屏,由 AppShell 统一控制背光。
constexpr int kScreenDimIdleSec = 60;
constexpr int kScreenOffIdleSec = 120;
// 降暗档背光亮度(百分比)。熄屏为 0。全亮时用 RestoreBrightness 恢复用户设定值。
constexpr uint8_t kScreenDimBrightness = 20;
...
void WakeFromScreenOffAction() { AppShell::GetInstance().WakeFromScreenOff(); }
...
void OnWakeFromScreenOff(lv_event_t*) { Schedule(WakeFromScreenOffAction); }
...
void AppShell::Tick() {
    ...
    // 触摸活动即视为唤醒:清空空闲计时,并在已降暗/熄屏时恢复全亮。
    // 否则只要不在手动屏保、不在播放音乐/播客、AI 未活跃,就累加空闲秒数,
    // 由 UpdateScreenPower() 据此推进降暗/熄屏。播放与屏保因此永不熄屏。
    const bool media_playing = server_media_playing ||
                               AppLocalMusicPlayer::GetInstance().playing();
    if (IsInputPressed()) {
        idle_seconds_ = 0;
        if (screen_power_ != ScreenPower::kActive) {
            WakeDisplayForEvent("触摸");
        }
    } else if (!IsAiActive() && page_ != Page::kScreensaver && !media_playing) {
        idle_seconds_++;
    }
...
void AppShell::WakeDisplayForEvent(const std::string& reason) {
    ...
    idle_seconds_ = 0;
    // 显式恢复全亮:板级 PowerSaveTimer 已不再控制背光,唤醒时由此处统一复位。
    ApplyScreenPower(ScreenPower::kActive);
...
void AppShell::WakeFromScreenOff() {
    // 熄屏吞噬层点击的落点:仅唤醒亮屏,不触发下方页面按钮。
    WakeDisplayForEvent("触摸");
}

void AppShell::UpdateScreenPower() {
    // 根据空闲秒数推进档位。屏保页不参与(手动全亮),其空闲计数在 Tick 中已停增。
    ScreenPower target = ScreenPower::kActive;
    if (page_ != Page::kScreensaver) {
        if (idle_seconds_ >= kScreenOffIdleSec) {
            target = ScreenPower::kOff;
        } else if (idle_seconds_ >= kScreenDimIdleSec) {
            target = ScreenPower::kDim;
        }
    }
    ApplyScreenPower(target);
}

void AppShell::ApplyScreenPower(ScreenPower level) {
    if (level == screen_power_) {
        return;
    }
    screen_power_ = level;
    // 该方法可能从未持锁的 WakeDisplayForEvent 调入,也可能从已持锁的
    // Tick/UpdateScreenPower 调入。端口锁为递归锁,统一在此加锁最安全,
    // 保证操作吞噬层(LVGL 对象)时始终持有 DisplayLockGuard。
    DisplayLockGuard lock(display_);
    auto* backlight = Board::GetInstance().GetBacklight();
    switch (level) {
        case ScreenPower::kActive:
            if (backlight != nullptr) {
                backlight->RestoreBrightness();
            }
            if (screen_off_swallow_ != nullptr) {
                lv_obj_add_flag(screen_off_swallow_, LV_OBJ_FLAG_HIDDEN);
            }
            break;
        case ScreenPower::kDim:
            if (backlight != nullptr) {
                backlight->SetBrightness(kScreenDimBrightness);
            }
            if (screen_off_swallow_ != nullptr) {
                lv_obj_add_flag(screen_off_swallow_, LV_OBJ_FLAG_HIDDEN);
            }
            break;
        case ScreenPower::kOff:
            if (backlight != nullptr) {
                backlight->SetBrightness(0);
            }
            // 熄屏时亮出顶层吞噬层:第一次触摸只用于唤醒,不穿透到下方按钮。
            if (screen_off_swallow_ != nullptr) {
                lv_obj_clear_flag(screen_off_swallow_, LV_OBJ_FLAG_HIDDEN);
                lv_obj_move_foreground(screen_off_swallow_);
            }
            break;
    }
}
...
void AppShell::EnsureUi() {
    ...
    // 熄屏吞噬层:全屏透明、置于顶层、平时隐藏。熄屏后亮出,拦截第一次触摸
    // 只做唤醒、不穿透到下方按钮(满足"熄屏点一下只唤醒不误触")。
    screen_off_swallow_ = lv_obj_create(lv_layer_top());
    lv_obj_remove_style_all(screen_off_swallow_);
    lv_obj_clear_flag(screen_off_swallow_, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_set_size(screen_off_swallow_, display_->width(), display_->height());
    lv_obj_align(screen_off_swallow_, LV_ALIGN_CENTER, 0, 0);
    lv_obj_set_style_bg_opa(screen_off_swallow_, LV_OPA_TRANSP, 0);
    lv_obj_add_flag(screen_off_swallow_, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_add_flag(screen_off_swallow_, LV_OBJ_FLAG_HIDDEN);
    lv_obj_add_event_cb(screen_off_swallow_, OnWakeFromScreenOff, LV_EVENT_PRESSED, nullptr);
}
