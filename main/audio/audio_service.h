...
public:
    ...
    bool PlayOggFile(const std::string& path, std::function<bool()> should_stop = nullptr);
    bool PlayOggUrl(const std::string& url, std::function<bool()> should_stop = nullptr);
    bool PlayMp3File(const std::string& path, std::function<bool()> should_stop = nullptr);
    bool PlayMp3Url(const std::string& url, std::function<bool()> should_stop = nullptr);

    // Current media playback progress. Duration is 0 when the source does not
    // provide a reliable total.
    int media_position_ms() const { return media_position_ms_.load(); }
    int media_duration_ms() const { return media_duration_ms_.load(); }
...
private:
    ...
    void ResetMediaProgress(int duration_ms = 0);
    void AdvanceMediaProgress(int delta_ms);
    bool PlayMp3Uri(const std::string& uri, const char* label, std::function<bool()> should_stop,
                    int file_bytes = 0, int preset_duration_ms = 0);
    void OutputMp3Pcm(const uint8_t* data, int data_size);
    static int Mp3OutputCallback(uint8_t* data, int data_size, void* ctx);
    static int Mp3EventCallback(esp_asp_event_pkt_t* event, void* ctx);
};
